  ...
  create: Components.detect((context, components) => {
    ...
    function getRefPropIndexes(method) {
      ...
      if (method.static) {
        ...
        if (staticIndex >= 0) {
          ...
          groupNames.push('static-methods');
        }
      }
      ...
      if (method.typeAnnotation) {
        ...
        if (annotationIndex >= 0) {
          ...
          groupNames.push('type-annotations');
        }
      }
      ...
      if (indexes.length === 0) {
        for (i = 0, j = methodsOrder.length; i < j; i++) {
          ...
          if (matching) {
            indexes.push(i);
            groupNames.push('match ' + methodsOrder[i]);
          }
        }
      }
      ...
      if (indexes.length === 0 && method.instanceVariable) {
        ...
        if (annotationIndex >= 0) {
          indexes.push(annotationIndex);
          groupNames.push('instance-variables');
        }
      }
      ...
      if (indexes.length === 0 && method.instanceMethod) {
        ...
        if (annotationIndex >= 0) {
          indexes.push(annotationIndex);
          groupNames.push('instance-methods');
        }
      }
      ...
      if (indexes.length === 0) {
        for (i = 0, j = methodsOrder.length; i < j; i++) {
          if (methodsOrder[i] === 'everything-else') {
            indexes.push(i);
            groupNames.push('everything-else');
            break;
          }
        }
      }
      ...
      if (indexes.length === 0) {
        ...
        groupNames.push('Infinity');
      }

      if (!loggedMethods.includes(method.name)) {
        console.log('-'.repeat(50));
        console.log({ method, indexes, groupNames });
        loggedMethods.push(method.name);
      }
    ...
    function checkPropsOrder(properties) {
      const propertiesInfos = properties.map(node => ({
        nodeType:       node.type,
        nodeValueType: (node.value && node.value.type),
    ...
    return {
      'Program:exit': function() {
        ...
        for (const component in list) {
          ...
          console.log(list[component].node);
